...
2. Чтобы подтянуть обновления проекта достаточно через командную строку перейти в папку с проектом, выполнить 
```
    git pull
```
затем активировать виртуальное окружение (п. 1.5) и снова запустить приложение (п. 1.7)