...
@login_app.route("/")
@login_app.route("/login_app")
@check_user_valid
def login():
    return redirect(url_for('main.index'))


@login_app.route("/login_app", methods=['POST'])
def on_login():
    if login_user(request.form.get('login_app'), request.form.get('password')):
        return redirect(url_for('main.index'))
    else:
        session['login_message'] = "Неверный логин или пароль"
        return redirect(url_for('login_app.login_app'))
